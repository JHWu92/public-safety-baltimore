...
def bnia_fpn(data, ys, bower, kde200, xday=2, save_file=True):
    ...
    for hname, hfunc in hotspot_def:
        for fname in ['fp', 'fn', 'tp'][2:]:
            print(f"working on {xday}day, hotspot:{hname}, {fname}")
            fpn_res = {}
            for p in ys.columns:
                p_fpn_res = {}
...
bower_7d = pd.read_pickle('exp_res/bower_7day_raw_bower.pickle').to_dense()
ys_7d = pd.read_pickle('exp_res/bower_7day_raw_y.pickle').to_dense()
kde200_7d = pd.read_pickle('exp_res/bower_7day_raw_kde200.pickle')
bnia_fpn(d_bower, ys_7d, bower_7d, kde200_7d, 7, save_file=True)
